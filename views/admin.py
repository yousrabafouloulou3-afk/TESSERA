...
def save_professors(profs_data):
    ...
    c = conn.cursor()
    try:
        c.execute("SELECT DISTINCT specialite FROM Profs WHERE specialite IS NOT NULL")
        existing_specs_in_db = [row['specialite'] for row in c.fetchall()]
        ...
        for p in profs_data:
            ...
            if name_stripped:
                ...
                new_specs.add(p['specialite'])
                
        all_specs_to_process = set(existing_specs_in_db).union(new_specs)
...
def show_student_data_entry():
    ...
    for spec in existing_specs:
        ...
        data.append({
            "Specialty Name": format_specialty_with_acronym(spec['name']),
    ...
    if not data:
        data.append({
            ...
            "Number of Groups per Section": 2
        })
    ...
    config = {
        "Specialty Name": st.column_config.TextColumn(tr("Specialty Name"), disabled=False, required=True),
    ...
    edited_df = st.data_editor(df, column_config=config, use_container_width=True, hide_index=True, num_rows="dynamic", key=f"editor_{level}")
...
def show_professor_data_entry():
    ...
    existing_deps = list(profs_by_spec.keys())
    
    if not draft:
        draft = {}
        draft["num_deps"] = max(1, len(existing_deps))
        for i, dep in enumerate(existing_deps):
            draft[f"dep_name_{i}"] = dep
            
            spec_profs = profs_by_spec.get(dep, [])
            draft[f"num_profs_{i}"] = max(1, len(spec_profs))
            for j, p in enumerate(spec_profs):
                draft[f"pname_{i}_{j}"] = p['nameP']
                draft[f"mat_{i}_{j}"] = p['matricule'] if p['matricule'] else ""
                draft[f"isprof_{i}_{j}"] = bool(p['prof'])
    ...
    num_deps = st.number_input(tr("Number of departments"), min_value=1, value=int(draft.get("num_deps", 1)), step=1, key=f"num_deps_v{prof_ver}")
    current_draft["num_deps"] = num_deps
    
    departments = []
    for i in range(num_deps):
        dep_name = st.text_input(f"{tr('Department')} {i+1} - {tr('Name')}", value=draft.get(f"dep_name_{i}", ""), key=f"dep_name_{i}_v{prof_ver}")
        current_draft[f"dep_name_{i}"] = dep_name
        departments.append((i, dep_name))
        
    st.markdown("---")
    
    for i, dep_name in departments:
        if not dep_name.strip():
            continue
            
        st.markdown(f"#### 🏢 Department: `{dep_name}`")
        num_prof_draft_key = f"num_profs_{i}"
        num_prof_widget_key = f"{num_prof_draft_key}_v{prof_ver}"
        num_profs = st.number_input(f"{tr('Number of professors in ')}{dep_name}", min_value=0, value=draft.get(num_prof_draft_key, 1), step=1, key=num_prof_widget_key)
        current_draft[num_prof_draft_key] = num_profs
        
        for j in range(int(num_profs)):
            c1, c2, c3 = st.columns([2, 2, 1])
            pname_draft_key = f"pname_{i}_{j}"
            mat_draft_key = f"mat_{i}_{j}"
            isprof_draft_key = f"isprof_{i}_{j}"
            ...
            with c1:
                p_name = st.text_input(f"{tr('Professor')} {j+1} - {tr('Name')}", value=draft.get(pname_draft_key, ""), key=pname_widget_key)
