        ...
        if allow_prefs:
            if not my_mod_list:
                ...
                if st.button(tr("Upload Preferences Logic"), type="primary"):
                    ...
                    c.execute("INSERT INTO Preferences (ID_P, ID_M, t, score) VALUES (?, ?, ?, ?) ON CONFLICT (ID_P, ID_M, t) DO UPDATE SET score=EXCLUDED.score", (teacher_id, mod_id, t1, 0))
                    c.execute("INSERT INTO Preferences (ID_P, ID_M, t, score) VALUES (?, ?, ?, ?) ON CONFLICT (ID_P, ID_M, t) DO UPDATE SET score=EXCLUDED.score", (teacher_id, mod_id, t2, 10))
                    c.execute("INSERT INTO Preferences (ID_P, ID_M, t, score) VALUES (?, ?, ?, ?) ON CONFLICT (ID_P, ID_M, t) DO UPDATE SET score=EXCLUDED.score", (teacher_id, mod_id, t3, 20))
                    conn.commit()
                    conn.close()
